/**
 * IncomingCallModal – overlay hiển thị khi có cuộc gọi đến
 */
...
export default function IncomingCallModal() {
  ...
  const label  = callType === 'video' ? 'Gọi video đến' : 'Gọi thoại đến';
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', marginBottom: 4 }}>
          ...
          <div style={{
            ...
            background: callType === 'video' ? '#00b4d8' : '#3ba55c',
            borderRadius: '50%', width: 26, height: 26,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            border: '2px solid var(--bg-secondary)',
          }}>
            {callType === 'video' ? <Video size={13} color="#fff" /> : <Phone size={13} color="#fff" />}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 28, marginTop: 16 }}>
          ...
          <button onClick={rejectCall} style={{
            ...
            background: '#ed4245', cursor: 'pointer',
          ...
          <button onClick={answerCall} style={{
            width: 62, height: 62, borderRadius: '50%', border: 'none',
            background: '#3ba55c', cursor: 'pointer',
            display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4,
            color: '#fff', transition: 'background 0.15s',
          }}
        ...
        <div style={{ display: 'flex', gap: 36, marginTop: 4 }}>
          ...
          <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Chấp nhận</span>
        </div>
      </div>

      <style>{`
        @keyframes fadeIn { from { opacity: 0; transform: scale(0.92); } to { opacity: 1; transform: scale(1); } }
        @keyframes pulse  { 0%,100% { transform: scale(1); opacity: 0.15; } 50% { transform: scale(1.3); opacity: 0.08; } }
      `}</style>
    </div>
  );
}
