...
import {
  ...
  Shield, UserPlus, Crown, UserCog, Trash2, Sparkles,
} from 'lucide-react';
...
export default function RightSidebar({
  ...
  const handleAiSummary = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || 'Không thể tóm tắt, thử lại sau';
      setAiError(msg);
      setAiSummary(null);
    } finally {
      setAiLoading(false);
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                          <div style={{ minWidth: 0 }}>
                            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: 8 }}>
                              ...
                              {isSelf && <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>(bạn)</span>}
                            </div>
                ...
                {initialUnreadCount > 0 && (
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: aiLoading ? 'none' : '0 2px 10px rgba(108,99,255,0.35)',
                    }}
                  >
                    <Sparkles size={15} style={{ flexShrink: 0, animation: aiLoading ? 'spin 1s linear infinite' : 'none' }} />
                    <span>
                      {aiLoading
                        ? 'AI đang tóm tắt...'
                        : `✨ Tóm tắt ${initialUnreadCount} tin chưa đọc`}
                    </span>
                  </button>
                )}
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <span style={{ fontSize: 12, fontWeight: 800, color: '#a78bfa', textTransform: 'uppercase', letterSpacing: '0.6px' }}>
                        AI Tóm tắt
                      </span>
                    ...
                    <button
                      onClick={() => { setShowAiModal(false); setAiSummary(null); setAiError(''); }}
                  ...
                  {aiLoading && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0' }}>
                      ...
                      <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>Gemini đang phân tích tin nhắn...</span>
                    </div>
                  )}

                  {/* Error */}
                  {!aiLoading && aiError && (
                    <div style={{ fontSize: 13, color: '#ed4245', lineHeight: 1.5 }}>{aiError}</div>
                  )}

                  {/* No unread */}
                  {!aiLoading && !aiError && aiSummary?.reason === 'no_unread' && (
                    <div style={{ fontSize: 13, color: 'var(--text-muted)', fontStyle: 'italic' }}>
                      Bạn đã đọc hết tin nhắn trong cuộc trò chuyện này.
                    </div>
                  )}

                  {/* Summary text */}
                  {!aiLoading && !aiError && aiSummary?.summary && (
                    <>
                      <div style={{
                        fontSize: 13,
                      ...
                      <div style={{
                        ...
                        <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                          {aiSummary.reason === 'cached' ? '⚡ Từ cache' : '🤖 Powered by Gemini 2.0 Flash'}
                          {aiSummary.totalSummarized ? ` · ${aiSummary.totalSummarized} tin` : ''}
                        </span>
                        <button
                          onClick={handleAiSummary}
                          disabled={aiLoading}
                          style={{
                            ...
                            cursor: 'pointer', fontSize: 11, color: '#a78bfa', fontWeight: 600,
                          }}
                        >
                          Làm mới
                        </button>
