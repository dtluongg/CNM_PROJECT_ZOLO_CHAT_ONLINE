 ...
 * Fixes: useFocusEffect for fresh server data, Supabase Storage upload, modern UI
 */
    ...
    <View style={{ width: size, height: size }}>
      ...
      {status && (
        <View style={{
          ...
          borderRadius: dotSize / 2, backgroundColor: sc,
          bottom: 0, right: 0,
          borderWidth: 2.5, borderColor: THEME.bgSecondary,
        }} />
      )}
    </View>
  );
...
export default function ProfileScreen({ navigation }) {
  ...
  const fetchProfile = async (isRefresh = false) => {
    ...
    try {
      ...
      // Only use cached user as last resort display (don't overwrite a good profile)
      if (!profile && user) {
        setProfile(user);
  ...
  const handlePickAvatar = async () => {
    ...
    const result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ImagePicker.MediaTypeOptions.Images, // ✅ sửa
    ...
    try {
      ...
      setProfile(updated);
      await saveProfile({ avatar: url }, updated);
    } catch (e) {
      console.error('[avatar upload]', e); // ✅ log lỗi thật để debug
  ...
  const handlePickBanner = async () => {
    ...
    try {
      ...
      setProfile(updated);
      await saveProfile({ banner: url }, updated);
    } catch (e) {
      console.error('[banner upload]', e); // ✅ log lỗi thật
  ...
  // ── Save to backend ──────────────────────────────────────────────
  const saveProfile = async (data, optimisticProfile) => {
    const previousProfile = profile; // ✅ capture trước khi thay đổi
    setSaving(true);
    try {
      ...
      if (optimisticProfile) setProfile(previousProfile); // ✅ rollback đúng
        ...
        >
            ...
            {d.banner
              ...
              : (
                <View style={[s.bannerImg, {
                  backgroundColor: d.usernameColor || THEME.accent,
                }]} />
              )
            }
          ...
          <View style={s.avatarSection}>
            <TouchableOpacity onPress={handlePickAvatar} style={s.avatarWrap}>
              {uploadingAvatar ? (
                ...
                <View style={[s.avatarRing, { borderColor: statusInfo.color }]}>
                  <Avatar name={d.displayName} avatar={d.avatar} size={76} />
                </View>
              )}
              <View style={s.cameraIcon}><Text style={{ fontSize: 12 }}>📷</Text></View>
            </TouchableOpacity>

            <View style={s.nameBlock}>
              <Text style={[s.displayName, { color: d.usernameColor || THEME.textPrimary }]} numberOfLines={1}>
                {d.displayName || 'Tên hiển thị'}
              </Text>
              {d.username && (
                <Text style={s.handle}>@{d.username}</Text>
              )}
            </View>
            ...
            >
              ...
              <Text style={[s.statusChipText, { color: statusInfo.color }]}>
                {statusInfo.label}
              </Text>
                ...
                <View style={s.infoRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={s.infoRowValue}>
                      {new Date(d.createdAt).toLocaleDateString('vi-VN', { year: 'numeric', month: 'long', day: 'numeric' })}
                    </Text>
                  </View>
                </View>
              </>
        ...
        <ScrollView contentContainerStyle={s.qrScroll}>
          <View style={s.qrCard}>
            ...
            <View style={[s.qrBand, { backgroundColor: d.usernameColor || THEME.accent }]}>
              <Avatar name={d.displayName} avatar={d.avatar} size={64} status={d.status} />
            </View>
            ...
            <View style={s.qrBody}>
              ...
              <Text style={s.qrCaption}>
                Quét mã này để xem hồ sơ của {d.displayName}
              </Text>
          ...
          <View style={s.sheet}>
            ...
            {STATUS_OPTIONS.map(st => {
              ...
              return (
                ...
                >
                  <View style={[s.statusDot, { backgroundColor: info.color, width: 12, height: 12, borderRadius: 6 }]} />
                  <Text style={[s.statusOptionText, active && { color: info.color, fontWeight: '700' }]}>
                    {info.label}
                  </Text>
      ...
      <Modal visible={colorModal} animationType="slide" transparent>
        <View style={s.overlay}>
          <View style={s.sheet}>
            ...
            <Text style={{ color: d.usernameColor || THEME.accent, fontSize: 20, fontWeight: '800', textAlign: 'center', marginBottom: 16 }}>
              {d.displayName}
            </Text>
      ...
      <Modal visible={logoutModal} animationType="fade" transparent>
        <View style={s.overlay}>
          <View style={[s.sheet, { paddingBottom: 28 }]}>
            ...
            <Text style={{ color: THEME.textMuted, textAlign: 'center', marginBottom: 24, lineHeight: 20 }}>
  ...
  return (
    <TouchableOpacity style={s.settingRow} onPress={onPress} activeOpacity={0.7}>
      ...
      <View style={{ flex: 1 }}>
        ...
        {sub && (
          <Text style={[s.settingSub, accent && { color: accent }]} numberOfLines={1}>{sub}</Text>
        )}
      </View>
...
const s = StyleSheet.create({
  ...
  loadingScreen: {
    ...
    backgroundColor: THEME.bgPrimary, gap: 12,
  },
  ...
  bannerEditHint: {
    ...
    backgroundColor: 'rgba(0,0,0,0.52)', borderRadius: 16,
  ...
  qrBand: {
    height: 88, justifyContent: 'flex-end',
    alignItems: 'flex-start', paddingHorizontal: 20, paddingBottom: -32,
  },
  ...
  fieldInput: {
    backgroundColor: THEME.bgInput, borderRadius: 10, padding: 13,
  ...
  saveBtn: {
    flex: 1, backgroundColor: THEME.accent, borderRadius: 12,
    paddingVertical: 14, alignItems: 'center',
  },
  ...
  // Color grid
  colorGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 14, justifyContent: 'center', marginBottom: 20 },
  colorSwatch: {
