...
export const CallProvider = ({ children }) => {
  ...
  const initiateCall = useCallback(async (targetUser, type) => {
    ...
    // Phase 1: Ring ngay lập tức, không cần đợi media/offer
    socketRef.current.emit('call:ring', { calleeId: targetUser._id, type }, async (res) => {
      if (res?.error) {
        ...
        Alert.alert(
          ...
          isOffline ? `${targetUser.displayName || 'Người dùng'} đang không online` : res.error,
        );
        return;
      }
      ...
      // Phase 2: Setup media + offer song song (callee đã đổ chuông rồi)
      try {
        await createPeer();
  ...
  const answerCall = useCallback(async () => {
    ...
    try {
      ...
      if (immediateOffer) {
        ...
        // Flow mới (call:ring): báo callee accept, chờ offer qua call:offer event
        socketRef.current.emit('call:accept', { callId: cid }, async (res) => {
          ...
          setRemoteUser(callerInfo);
          startTimer();
          // Offer sẽ đến qua socket event 'call:offer', handled in socket listener below
        });
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    // Flow mới: caller nhận khi callee accept (trước khi có answer)
    ...
    // Flow mới: callee nhận offer riêng sau khi accept
